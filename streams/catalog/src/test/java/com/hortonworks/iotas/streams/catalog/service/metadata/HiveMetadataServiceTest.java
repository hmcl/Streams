...
public class HiveMetadataServiceTest {
        ...
        for (String database : HIVE_TEST_DATABASES) {
            ...
            for (String table : HIVE_TEST_TABLES) {
                hiveService.createTable(table, database, "owner", (int)System.currentTimeMillis(), (int)System.currentTimeMillis(), 
                        10, getStorageDescriptor(), Collections.emptyList(), new HashMap<>(), "orig", "expanded", "STRING");
            }
        }
    }
    ...
    private Map<String, String> getHiveSiteConfig() throws IOException {
        return new ObjectMapper().readValue(Thread.currentThread().getContextClassLoader()
                .getResourceAsStream(HIVE_SITE_CONFIG),
