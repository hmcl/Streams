...
public class StorablePreparedStatement {
    protected static final Logger log = LoggerFactory.getLogger(PreparedStatementBuilder.class);
    protected PreparedStatement preparedStatement;
    private int numParams;                          // Number of prepared statement parameters

    public StorablePreparedStatement(Connection connection, JdbcStorageManagerConfig config,
                                     SqlBuilder sqlBuilder) throws SQLException {
        prepareStatement(connection, config, sqlBuilder);
        setNumParams(sqlBuilder);
    }

    private void prepareStatement(Connection connection, JdbcStorageManagerConfig config,
                                  SqlBuilder sqlBuilder) throws SQLException {
    ...
    public PreparedStatement getPreparedStatement(SqlBuilder sqlBuilder) throws SQLException {
        ...
        if (columns != null) {
            ...
            int nTimes = len % numParams;   // Number of times each column must be replace on a query parameter
