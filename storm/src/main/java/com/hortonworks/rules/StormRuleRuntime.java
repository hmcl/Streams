...
public class StormRuleRuntime implements RuleRuntime<Tuple, IOutputCollector> {
    private Rule<Schema.Field> rule;
    private Script<Tuple, Schema.Field> script;     // Script used to evaluate the condition

    public StormRuleRuntime (Rule<Schema.Field> rule, Script<Tuple, Schema.Field> script) {
        this.rule = rule;
        this.script = script;
